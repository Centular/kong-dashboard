...
//Set the kong url if passed through command line
if (kong_url) {
  webapp.use(function*(next) {
    //add http if only the host is passed
    if (kong_url.toLowerCase().indexOf("http") == -1) {
      kong_url = "http://" + kong_url;
    }

    //add " to the back and front of string to make it readable as json
    if (kong_url.toLowerCase().indexOf("\"") == -1) {
        kong_url = '"' +kong_url+ '"'
    }

    //check if the cookie exists or different to current cookie
    if((!this.cookies.get('config.url'))||(this.cookies.get('config.url').localeCompare(kong_url))){
      this.cookies.set('config.url', kong_url, {
...
console.log('Kong url set to :' + kong_url);
